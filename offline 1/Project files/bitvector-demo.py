...
int_val = b.intValue()
s = Sbox[int_val]  # get the sbox value
...
def divide_words(key):
    words = []
    for i in range(len(key) // 16):  # floordivide
        x = key[i * 16: i * 16 + 16]  # first
        grid = [[], [], [], []]
        for j in range(4):
            for k in range(4):
                grid[j].append(x[j + k * 4])
        words.append(grid)
    return words
...
def key_expansion(key):
    rc=1
    round_constants = [[rc, 0, 0, 0]]
    for i in range(9):
        rc *= 2
        if rc>0x80:
            rc ^= 0x11b
        round_constants.append([rc, 0, 0, 0])
