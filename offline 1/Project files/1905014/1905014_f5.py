...
def rotate_left(row_, n=1):
    return row_[n:] + row_[:n]
...
def encrypt(plain_blocks, iv_given, all_keys):
    crypt = []
...
def decrypt(crypt, iv_given, all_keys):
    decrypting = []
    iv_hex = iv_given.encode("utf-8").hex(sep=",").rjust(2, '0')
    separate = iv_hex.split(",")
    iv_mat = [[0 for _ in range(4)] for _ in range(4)]
...
def modular_sqrt(s1, s2):
    if legendre_symbol(s1, s2) != 1:
        return 0
    elif s1 == 0:
        return 0
    elif s2 == 2:
    ...
    n = 2
    while legendre_symbol(n, s2) != -1:
        n += 1

    x1 = pow(s1, int((s + 1) // 2), s2)
    b1 = pow(s1, int(s), s2)
    g = pow(n, int(s), s2)
    r = e
...
def get_params():
    p_curve= 284905286209134604369454064497384132731
    x_curve = 29594750178373188016493346073486860970
    y_curve = 266375854489868075368855722121394270880
    a = 2
    b = 2
    return p_curve, x_curve, y_curve
...
def generate_coordinates(p, a_given, b_given):
    x_point = random.randrange(1, p - 1)
    y_point = get_y_in_curve(x_point, a_given, b_given, p)
    while y_point is None:
        x_point = random.randrange(1, p - 1)
        y_point = get_y_in_curve(x_point, a_given, b_given, p)
...
def convert_key(key):
    ...
    for i in range(0, 32, 2):
        converted_key += new_key[i:i+2]
        converted_key += ","
    converted_key = converted_key[:-1]
